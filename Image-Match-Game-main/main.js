...
// ============================================================
// 「已刪除圖卡」本地遮蔽機制
// 目的：由於 Supabase Storage 的刪除權限可能未開放（DELETE policy），
// 或 list 有短期索引延遲，測試發現 remove() 即使回傳 200 也可能未真正刪除，
// 導致重新整理後失敗圖卡又跑出來。
// 因此刪除時會把檔名記到 localStorage，載入圖卡時過濾掉這些檔名，
// 達到「刪除後不再出現」的效果。預設遮蔽保留 24 小時。
// ============================================================
const DELETED_CARDS_KEY = 'deletedFlashcardNames';
const DELETED_CARDS_TTL_MS = 24 * 60 * 60 * 1000;

function getDeletedFlashcardNames() {
    try {
        const raw = localStorage.getItem(DELETED_CARDS_KEY);
        const arr = raw ? JSON.parse(raw) : [];
        const now = Date.now();
        // 移除已過期的記錄，避免永久遮蔽日後重新上傳的同名圖卡
        const fresh = arr.filter(e => e && e.t && (now - e.t) < DELETED_CARDS_TTL_MS);
        if (fresh.length !== arr.length) {
            localStorage.setItem(DELETED_CARDS_KEY, JSON.stringify(fresh));
        }
        return fresh.map(e => e.n);
    } catch (e) {
        console.warn('讀取已刪除圖卡快取失敗:', e);
        return [];
    }
}

function markFlashcardDeleted(fileName) {
    if (!fileName) return;
    try {
        const arr = getDeletedFlashcardNames();
        if (!arr.includes(fileName)) {
            const saved = JSON.parse(localStorage.getItem(DELETED_CARDS_KEY) || '[]')
                .filter(e => e && e.n !== fileName);
            saved.push({ n: fileName, t: Date.now() });
            localStorage.setItem(DELETED_CARDS_KEY, JSON.stringify(saved));
            console.log('已將圖卡加入遮蔽清單:', fileName);
        }
    } catch (e) {
        console.warn('儲存已刪除圖卡快取失敗:', e);
    }
}

// ============================================================
// 統一的圖卡刪除處理
// 即使雲端 remove 因權限失敗，也會把卡片從資料陣列 / DOM / 遮蔽清單移除，
// 確保當次與之後的載入都不會再顯示。
// ============================================================
async function deleteFlashcardForever(fileName, cardElement) {
    let storageWarn = null;
    try {
        const { error: storageError } = await supabaseClient.storage
            .from('images')
            .remove([fileName]);
        if (storageError) {
            storageWarn = storageError;
            console.warn('Storage 刪除回傳錯誤，將改為隱藏:', fileName, storageError);
        }
    } catch (e) {
        storageWarn = e;
        console.error('Storage 刪除拋出例外，將改為隱藏:', fileName, e);
    }

    // 盡力刪除資料庫的解釋資料（ISOLATED 或已無記錄都不影響）
    try {
        ...
        if (dbError && dbError.code !== 'PGRST116') {
            console.warn('刪除解釋資料時出現警告:', fileName, dbError);
        }
    } catch (e) {
        console.warn('刪除解釋資料時拋出例外:', fileName, e);
    }

    // 加入遮蔽清單，確保重新整理後不再出現
    markFlashcardDeleted(fileName);

    // 從全域資料陣列移除
    if (Array.isArray(allFlashcards)) {
        allFlashcards = allFlashcards.filter(f => f && f.fileName !== fileName);
    }
    ...
    if (storageWarn) {
        console.warn('雲端檔案可能未刪除（權限不足），已在本機隱藏該圖卡:', fileName);
        showTemporaryMessage('圖卡已隱藏（雲端同步可能未完成）', 'error');
    } else {
...
// 一鍵隱藏所有「圖片載入失敗」的圖卡
function clearAllFailedFlashcards() {
    const failed = document.querySelectorAll('.flashcard.flashcard-load-failed');
    if (!failed.length) {
        showTemporaryMessage('目前沒有載入失敗的圖卡', 'success');
        return;
    }
    const names = [];
    failed.forEach(card => {
        const fn = card.dataset.fileName || card.dataset.filename;
        if (fn) {
            names.push(fn);
        }
    });
    names.forEach(n => markFlashcardDeleted(n));
    if (Array.isArray(allFlashcards)) {
        allFlashcards = allFlashcards.filter(f => f && !names.includes(f.fileName));
    }
    failed.forEach(card => { if (card.parentNode) card.remove(); });
    showTemporaryMessage(`已隱藏 ${names.length} 張載入失敗的圖卡`, 'success');
}
...
async function loadFlashcards() {
    try {
        ...
        // 預處理所有圖片數據，但不載入圖片
        // 先過濾掉使用者已刪除（遮蔽）的檔名，避免重新整理後又出現
        const deletedFileNames = getDeletedFlashcardNames();
        const visibleFiles = files.filter(file => !deletedFileNames.includes(file.name));
        console.log(`載入 ${visibleFiles.length}/${files.length} 張圖卡（已遮蔽 ${files.length - visibleFiles.length} 張）`);

        allFlashcards = visibleFiles.map(file => {
...
function createLazyFlashcard(imageUrl, word, fileName, timestamp = Date.now(), appendToBottom = false) {
    ...
    deleteButton.onclick = async (e) => {
        ...
        deleteConfirmModal.show(confirmMessage, async () => {
            await deleteFlashcardForever(fileName, card);
        });
    };
    
    card.addEventListener('dblclick', (e) => {
        // 只在編輯模式下屏蔽可編輯字段的双击，避免和编辑冲突
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 一鍵隱藏全部載入失敗的圖卡
    const clearFailedCardsBtn = document.getElementById('clearFailedCards');
    if (clearFailedCardsBtn) {
        clearFailedCardsBtn.addEventListener('click', clearAllFailedFlashcards);
    }
